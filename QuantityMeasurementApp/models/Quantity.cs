...
namespace QuantityMeasurementApp.Models
{
        ...
        where TUnit : struct, Enum
    {
        ...
        public Quantity<TUnit> Subtract(Quantity<TUnit> other, TUnit targetUnit)
        {
            ...
            // Re-scale to target unit
            double converted = 0;
            if (targetUnit is LengthUnit l) converted = l.ConvertFromBase(diffInBase);
            else if (targetUnit is WeightUnit w) converted = w.ConvertFromBase(diffInBase);
            else if (targetUnit is VolumeUnit v) converted = v.ConvertFromBase(diffInBase);

            return new Quantity<TUnit>(Math.Round(converted, 2), targetUnit);
        }
        ...
        private void Validate(Quantity<TUnit> other)
        {
        ...
        public Quantity<TUnit> Add(Quantity<TUnit> other, TUnit target)
        {
            ...
            else if (target is VolumeUnit v) converted = VolumeUnitExtension.ConvertFromBase(v,baseSum);
