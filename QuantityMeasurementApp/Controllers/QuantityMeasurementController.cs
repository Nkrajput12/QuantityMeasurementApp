...
namespace QuantityMeasurementApp.Controllers
{
    public class QuantityMeasurementController
    {
        ...
        {
            Console.WriteLine("\n--- Operation History ---");
            Console.WriteLine("1. Database || 2. Cache");
            bool fromDatabase;
            ...
            var history = _service.GetHistory(fromDatabase);
            foreach(var record in history)
            {
                Console.WriteLine($"[{record.Timestamp}] {record.OperationType}: {record.InputDetails} => {record.Result}");
            }
        }
